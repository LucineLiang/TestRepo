...
% The transmission channel of this program is the general channel.
...
%% Channel: realistic channel(linear combination of different errors)
Error = {I,I,I,I,I,I,I,I,I};
% decide the position that the error occurs
error_position = randi(9)
% generate 4 random numbers
error_type = rand(4,1);
% calculate the summation of the 4 numbers above
error_sum = sum(error_type);
%calculate the percentage of each kind of error(all 4 kinds add up to 1)
for e = 1:4
    error_type(e) = error_type(e)/error_sum;
end
error_type
% write the linear combination of the error
W = sqrt(error_type(1))*I + sqrt(error_type(2))*Ux + sqrt(error_type(3))*Uz + sqrt(error_type(4))*(Ux*Uz);
% replace the corresponding position with the error
Error{error_position} = W;
...
for round = 1:8
    ...
    dist = rand(1);% randomly decide which state it collapses into
    if dist > probability(2)
        % the outcome will be like the first projection operator works
        % calculate the state after measurement
        sc_error_17 = (P{1} * sc_error_17)/sqrt(sc_error_17' * P{1} * sc_error_17);
        sc_error_17 = Recover{1} * sc_error_17;% trace out one qubit
        syndrome(9-round) = 0;% record the syndrome
    else % the outcome will be like the second projection operator works
        sc_error_17 = (P{2} * sc_error_17)/sqrt(sc_error_17' * P{2} * sc_error_17);
